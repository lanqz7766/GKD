...
from mmdet.utils.det_cam_visualizer import (DetAblationLayer, Student_DetCAMModel,
                                            ...
                                            FeatmapAM, reshape_transform, GradCAM, LayerCAM)

try:
    from pytorch_grad_cam import (AblationCAM, EigenGradCAM, GradCAMPlusPlus, XGradCAM)
except ImportError:
...
class GKD_DetectionDistiller(BaseDetector):
    ...
    def __init__(self,
        ...
        super(GKD_DetectionDistiller, self).__init__()
    ...
    def forward_train(self, img, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore=None, **kwargs):
        ...
        for image_path in images:
            ...
            student_grayscale_cam = self.student_det_cam_visualizer(
                ...
                aug_smooth=False,
                eigen_smooth=False) # (1, 375, 1242)
            # logger.info(f'grayscale_cam shape is {grayscale_cam[1].shape}') list[(1, 375, 1242)] -> len(5)
            # logger.info(f'grayscale_cam min is {np.min(grayscale_cam[1])}')
        ...
        for i, item_loc in enumerate(self.distill_cfg):
            ...
            for item_loss in item_loc.methods:
                ...
                student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat, teacher_cam_mask_per_layer[4-i], student_cam_mask_per_layer[4-i], gt_bboxes, img_metas)
