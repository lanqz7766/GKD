...
class Distiller(BaseDetector):
    ...
    def __init__(self,
                 ...
                 init_student=False,
                 is_layer_by_layer_distill=False,
                 use_teacher_group=False,
                 cl_head=dict(
                    type='MoCoCLHead',
                    img_channels=256,
                    pts_channels=256,
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        student_loss, student_all_stage_det_querys, all_stage_pos_assigned_gt_inds, student_features = self.student.forward_train_distill( \
                        img, img_metas, teacher_bboxes=None, teacher_labels=None, \
                         is_layer_by_layer_distill=False, **kwargs)
        ...
        #         student_loss[f'stage{stage}_relation_distill_loss'] = 0*stage_loss
        ...
        #         student_loss[f'stage{stage}_relation_distill_loss'] += 0*relation_distill_loss_stage
        """
        buffer_dict = dict(self.named_buffers())
        for item_loc in self.fgd_distill_cfg:
