...
class GKD_BMFI_FeatureLoss(nn.Module):
    ...
    def __init__(self,
        ...
        super(GKD_BMFI_FeatureLoss, self).__init__()
        self.temp = temp
    ...
    def forward(self,
        ...
        fg_loss, bg_loss = self.get_fea_loss(preds_S, preds_T, Mask_fg, Mask_bg, Mask_cam_T, Mask_cam_S,
                           C_attention_s, C_attention_t, S_attention_s, S_attention_t, self.temp)
        fea_loss = self.get_only_fea_loss(preds_S, preds_T, Mask_fg, Mask_bg, Mask_cam_T, Mask_cam_S,
                           C_attention_s, C_attention_t, S_attention_s, S_attention_t, self.temp)
        mask_loss = self.get_mask_loss(C_attention_s, C_attention_t, S_attention_s, S_attention_t)
        # rela_loss = self.get_rela_loss(preds_S, preds_T)
        cam_mask_loss = self.get_cam_mask_loss(Mask_cam_T, Mask_cam_S, preds_S)


        loss = self.alpha_fgd * fg_loss + self.beta_fgd * cam_mask_loss \
               + self.gamma_fgd * mask_loss + self.lambda_fgd * fea_loss
               # + self.lambda_fgd * rela_loss
            
        return loss


    def calculate_exponential(self, old_w, old_h, expand_factor = 2):
        w = 2*old_w
        h = 2*old_h
        cx = (w/2).int()
        cy = (h/2).int()
        ...
        gaussian_mask = torch.exp(-0.00005*(torch.square(y-y0)+torch.square(x-x0)))
    ...
    def get_fea_loss(self, preds_S, preds_T, Mask_fg, Mask_bg, Mask_cam_T, Mask_cam_S, C_s, C_t, S_s, S_t, temp):
    ...
    def get_only_fea_loss(self, preds_S, preds_T, Mask_fg, Mask_bg, Mask_cam_T, Mask_cam_S, C_s, C_t, S_s, S_t, temp):
        loss_mse = nn.MSELoss(reduction='sum')
        N, C, H, W= preds_S.shape
        Mask_fg = Mask_fg.unsqueeze(dim=1)
        Mask_bg = Mask_bg.unsqueeze(dim=1)
        ...
        fea_s = torch.mul(preds_S, torch.sqrt(S_t))
        fea_s = torch.mul(fea_s, torch.sqrt(C_t))
        fea_loss = loss_mse(fea_s, fea_t)/N/H/W
