...
class Distill_DETR_Head(DETRHead):
    ...
    def __init__(self,
        ...
        super(Distill_DETR_Head, self).__init__(
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        hs, init_reference, inter_references, *remaining = self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    mlvl_positional_encodings,
    ...
    def forward_train_distill(self,
        ...
        return losses, hs, pos_assigned_gt_inds_list_distill, outputs_classes, outputs_coords
    ...
    def simple_test_teacher_distill(self, mlvl_feats, img_metas, gt_bboxes=None, gt_labels=None, rescale=False, random_query = None, random_query_pos = None, random_reference_points = None):
...
#===================
        hs, init_reference, inter_references, \
        random_hs, random_init_reference, random_inter_references, \
        teacher_query, teacher_query_pos, teacher_reference_points, \
        enc_outputs_class, enc_outputs_coord = self.transformer.forward_teacher(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    mlvl_positional_encodings,
        ...
        outs = self.get_outputs(hs, init_reference, inter_references)
        random_outs = self.get_outputs(random_hs, random_init_reference, random_inter_references)

        return outs, random_outs, teacher_query, teacher_query_pos, teacher_reference_points
    ...
    def forward_train_distill_aux(self,
                            mlvl_feats,
                            img_metas,
                            gt_bboxes,
                            gt_labels=None,
        ...
        if not self.as_two_stage:
            query_embeds = self.query_embedding.weight
            # random_query_embeds = random_query_embeds

        hs, init_reference, inter_references, \
        random_hs, random_init_reference, random_inter_references, \
        teacher_hs, teacher_init_reference, teacher_inter_references, \
        enc_outputs_class, enc_outputs_coord = self.transformer(
                    mlvl_feats,
                    mlvl_masks,
                    query_embeds,
                    ...
                    teacher_query,
                    teacher_query_pos,
                    teacher_reference_points,
                    reg_branches=self.reg_branches if self.with_box_refine else None,  # noqa:E501
                    cls_branches=self.cls_branches if self.as_two_stage else None  # noqa:E501
            )
        # hs = hs.permute(0, 2, 1, 3)
        # random_hs = random_hs.permute(0, 2, 1, 3)
        # teacher_hs = teacher_hs.permute(0, 2, 1, 3)

        outs = self.get_outputs(hs, init_reference, inter_references)
        if random_hs is not None:
            random_outs = self.get_outputs(random_hs, random_init_reference, random_inter_references)
        else:
            random_outs = None

        if teacher_hs is not None:
            teacher_outs = self.get_outputs(teacher_hs, teacher_init_reference, teacher_inter_references)
        else:
            teacher_outs = None
        ...
        #                 gt_bboxes_ignore=gt_bboxes_ignore, is_layer_by_layer_distill=is_layer_by_layer_distill)
        # return losses, hs, pos_assigned_gt_inds_list_distill, outputs_classes, outputs_coords
        return losses, teacher_outs, random_outs


    def get_outputs(self, hs, init_reference, inter_references):
        hs = hs.permute(0, 2, 1, 3)
        outputs_classes = []
        outputs_coords = []
